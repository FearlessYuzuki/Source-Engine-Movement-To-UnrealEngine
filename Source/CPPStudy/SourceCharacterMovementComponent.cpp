	...
	float BrakingDeceleration)
{
	...
	if (bFluid)
	{
		Super::CalcVelocity(DeltaTime, Friction, bFluid, BrakingDeceleration); //TODO: AFter finished Ground Calc
	}
}
...
void USourceCharacterMovementComponent::WalkMove(float DeltaTime)
{
	...
	if (Velocity.Size2D()<DEFAULTSPEED)
	{
		bCanAccele = true;
	}
	else
	...
	float DeltaTime)
{
	...
	accelspeed = wishSpeed*DeltaTime*acceleration*GroundFriction;//Firction need RayTracing
